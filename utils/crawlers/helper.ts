...
export function filterDuplicate(newArr: CrawlerReturnObject[], oldArr: CrawlerReturnObject[]): CrawlerReturnObject[] {
  return newArr.filter((newArrItem) => {
    const find = oldArr.find(oldArrItem => serializeString(newArrItem.title) === serializeString(oldArrItem.title))
    return !find
  })
}

function serializeString(str: string) {
  // return getWordsInText(str)
  return str
}

const getWordsInText = function (text: string) {
